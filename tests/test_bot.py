...
def test_handle_trade_bare_chart_sent_when_file_exists(monkeypatch):
    ...
    assert ok is True
    assert len(photos) == 2                       # daily chart + weekly chart
    assert any("_weekly" in f for f in fetched)   # the weekly chart was fetched
...
def test_bare_trade_chart_photo_caption_is_raw_fired_line(monkeypatch):
    ...
    assert "weekly" in photos[1]        # weekly chart caption
